...
copy_file "$HOME/.config/kitty/kitty.conf" "config/kitty/kitty.conf" "kitty.conf"
echo ""
...
if [ -d "$HOME/fondos" ]; then
    cp "$HOME/fondos"/* "$DOTFILES_DIR/wallpapers/" 2>/dev/null
    # Buscar y renombrar el primer jpg a fondo.jpg si existe
    first_wallpaper=$(ls "$DOTFILES_DIR/wallpapers"/*.jpg 2>/dev/null | head -n1)
    if [ ! -z "$first_wallpaper" ]; then
        cp "$first_wallpaper" "$DOTFILES_DIR/wallpapers/fondo.jpg"
